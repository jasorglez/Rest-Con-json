...
uses
  ...
  Dialogs, jpeg, ExtCtrls,  ImgList, StdCtrls, Buttons,
...
begin
  if Sender.ClassType = TcxButton then
    TcxButton(Sender).Font.Style := TcxButton(Sender).Font.Style - [fsBold];
end;
...
begin
  ...
  for ii := 0 to Self.ComponentCount -1 do
    if (Self.Components[ii].ClassType = TcxButton ) and (TWinControl(Self.Components[ii]).Visible) then
