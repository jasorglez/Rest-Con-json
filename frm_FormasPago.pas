...
uses
  ...
  dxSkinVisualStudio2013Dark, dxSkinVisualStudio2013Light,  DBAccess, Uni, MemDS,
...
type
  TfrmFormasPago = class(TForm)
    ...
    pnl_Grid: TPanel;
    pnl_Datos: TPanel;
    grid_FormasPago3: TcxGridDBTableView;
    Grid_FormasPago2: TcxGridLevel;
    ...
    dtsformaspago: TUniDataSource;
    formapago: TFDMemTable;
...
uses Func_Genericas, global,  frmConexion;
{$R *.dfm}
...
begin
  ...
  if global_frmActivo = 'frm_pagos' then
  begin
     global_frmActivo := '';
    // dtmConexion.Formaspagos.refresh;
    // FrmOrdenesdeCompras.oc.FieldByName('iId_formapago').asInteger    :=  FormaPago.fieldbyname('iId').asInteger;
  end;
end;
...
begin
  ...
  dtmConexion.QryBusca.SQL.Add('select * from con_tesoreriaegresos where sFormaPago = :sFormaPago');
  dtmConexion.QryBusca.ParamByName('sFormaPago').AsInteger :=  FormaPago.FieldByName('sIdFormaPago').AsInteger;
...
//     frxFormasPagos.ShowReport(dtmConexion.configuracion.FieldByName('sFormatos').AsString, PermisosExportar(dtmConexion.zConnection, global_grupo, sMenuP))
    frxFormasPagos.ShowReport();
